...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, env):
        ...
        self.symmetric = dic_traffic_env_conf["SYMMETRIC"] if "SYMMETRIC" in dic_traffic_env_conf.keys() else False
    ...
    def transform_2dim(self, wsen_vehs):
        if self.current_phase_index < 0:
            wsen_vehs = [0 for _ in range(self.num_approaches*self.num_lanes)]

        if self.max_vehs > 1:
            wsen_vehs = [min(x/self.max_vehs, 1.0) for x in wsen_vehs]
        ...
        green = wsen_approach[self.current_phase_index]
        all = np.sum(wsen_approach)
        red = all - green
        if self.symmetric:
            red /= (self.num_approaches - 1)
        return [green, red]

    def _update_feature_map(self):
        dic_feature = dict()
        dic_feature["transform_2dim"] = self.transform_2dim(self._get_lane_num_vehicle(self.list_entering_lanes))
